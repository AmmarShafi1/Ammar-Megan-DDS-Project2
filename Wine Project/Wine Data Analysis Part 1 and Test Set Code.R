...
data2= read_excel("Wine Train.csv")
...
#ID and Predicted
wine_testing = california_data[-indxTrain, c("ID", "quality", "fixed acidity", "residual sugar", "pH", "alcohol")]
predicted_quality <- data.frame(
  ID = wine_testing$ID,                
  quality = knnPredict_final          
)
# Write to CSV
write.csv(predicted_quality, "test_set.csv", row.names = FALSE)
print("Test Set for Wine Quality CSV has been created: test_set.csv")
